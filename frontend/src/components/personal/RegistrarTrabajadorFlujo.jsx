...
import { 
  ...
  Layers, Check, Laptop, Database, Cpu, Wrench, FileCode, Users, Award
} from 'lucide-react';
...
export function RegistrarTrabajadorFlujo({ onVolver, onSuccess, lockRoleToDesarrollador = false }) {
  ...
  const [habilidadesDirectivas, setHabilidadesDirectivas] = useState([
    'Gestión de Proyectos', 'Scrum Master', 'Planificación WBS'
  ]);
  ...
  const handleSubmit = async (e) => {
    ...
    if (habilidadesTecnicas.length === 0 && habilidadesDirectivas.length === 0) {
      setFormError('Debe seleccionar al menos una habilidad o competencia.');
      return;
    }
  ...
  return (
    <div className="min-h-screen bg-zinc-50 dark:bg-zinc-950 p-4 sm:p-6 lg:p-8 space-y-6">
      ...
      {/* BARRA SUPERIOR DE NAVEGACIÓN Y ENCABEZADO */}
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 pb-4 border-b border-zinc-200 dark:border-zinc-800">
        <div className="flex items-center gap-3">
          <button
            onClick={onVolver}
            className="p-2.5 rounded-2xl bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 text-zinc-600 dark:text-zinc-400 hover:text-blue-600 hover:border-blue-300 dark:hover:border-blue-800 transition-all cursor-pointer shadow-sm"
          >
            <ArrowLeft size={18} />
          </button>
          <div>
            <div className="flex items-center gap-2">
              <span className="px-2.5 py-0.5 rounded-full text-[0.65rem] font-bold tracking-wider uppercase bg-blue-100 text-blue-800 dark:bg-blue-950 dark:text-blue-300 border border-blue-200 dark:border-blue-800">
                Alta Corporativa PostgreSQL
              </span>
              {lockRoleToDesarrollador && (
                <span className="px-2.5 py-0.5 rounded-full text-[0.65rem] font-bold uppercase bg-amber-100 text-amber-800 dark:bg-amber-950 dark:text-amber-300 border border-amber-200 dark:border-amber-800">
                  Exclusivo Desarrolladores
                </span>
              )}
            </div>
            <h1 className="text-xl sm:text-2xl font-extrabold text-zinc-900 dark:text-zinc-100 tracking-tight mt-1 flex items-center gap-2">
              Registrar Nuevo Colaborador <UserPlus size={22} className="text-blue-600" />
            </h1>
            <p className="text-xs text-zinc-500 font-medium">
              Flujo secuencial ordenado para enrolamiento de personal con soporte de perfil híbrido (Gestión + Técnica)
            </p>
          </div>
        </div>

        {/* INDICADOR DE PASOS */}
        <div className="flex items-center gap-2 bg-white dark:bg-zinc-900 p-1.5 rounded-2xl border border-zinc-200 dark:border-zinc-800 shadow-sm self-stretch sm:self-auto justify-around sm:justify-start">
          <div className={`flex items-center gap-2 px-3 py-1.5 rounded-xl text-xs font-bold transition-all ${activeStep >= 1 ? 'bg-blue-50 text-blue-700 dark:bg-blue-950 dark:text-blue-300' : 'text-zinc-400'}`}>
            <span className="w-5 h-5 rounded-full bg-blue-600 text-white text-[0.65rem] flex items-center justify-center font-mono">1</span>
            <span>Credenciales</span>
          </div>
          <ChevronRight size={14} className="text-zinc-400" />
          <div className={`flex items-center gap-2 px-3 py-1.5 rounded-xl text-xs font-bold transition-all ${activeStep >= 2 ? 'bg-blue-50 text-blue-700 dark:bg-blue-950 dark:text-blue-300' : 'text-zinc-400'}`}>
            <span className={`w-5 h-5 rounded-full text-[0.65rem] flex items-center justify-center font-mono ${activeStep >= 2 ? 'bg-blue-600 text-white' : 'bg-zinc-200 text-zinc-600 dark:bg-zinc-800'}`}>2</span>
            <span>Perfil</span>
          </div>
          <ChevronRight size={14} className="text-zinc-400" />
          <div className={`flex items-center gap-2 px-3 py-1.5 rounded-xl text-xs font-bold transition-all ${activeStep >= 3 ? 'bg-blue-50 text-blue-700 dark:bg-blue-950 dark:text-blue-300' : 'text-zinc-400'}`}>
            <span className={`w-5 h-5 rounded-full text-[0.65rem] flex items-center justify-center font-mono ${activeStep >= 3 ? 'bg-blue-600 text-white' : 'bg-zinc-200 text-zinc-600 dark:bg-zinc-800'}`}>3</span>
            <span>Habilidades WBS</span>
          </div>
        </div>
      </div>
      ...
      {/* FORMULARIO SECUENCIAL EN ALTURA */}
      <form onSubmit={handleSubmit} className="space-y-6 max-w-5xl mx-auto">
        
        {/* PASO 1: IDENTIFICACIÓN Y CREDENCIALES */}
        <div ref={step1Ref} className={`p-6 rounded-3xl bg-white dark:bg-zinc-900 border transition-all shadow-md space-y-5 ${activeStep === 1 ? 'border-blue-500 ring-2 ring-blue-500/20' : 'border-zinc-200 dark:border-zinc-800'}`}>
          <div className="flex justify-between items-center pb-3 border-b border-zinc-100 dark:border-zinc-800">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-2xl bg-blue-50 dark:bg-blue-950 text-blue-600 dark:text-blue-400 flex items-center justify-center font-extrabold text-sm border border-blue-200 dark:border-blue-800">
                1
              </div>
              <div>
            ...
            <span className="text-[0.68rem] font-bold font-mono px-3 py-1 rounded-full bg-blue-50 text-blue-700 dark:bg-blue-950 dark:text-blue-300 border border-blue-200 dark:border-blue-800">
        ...
        <div ref={step2Ref} className={`p-6 rounded-3xl bg-white dark:bg-zinc-900 border transition-all shadow-md space-y-5 ${activeStep === 2 ? 'border-blue-500 ring-2 ring-blue-500/20' : 'border-zinc-200 dark:border-zinc-800'}`}>
          <div className="flex justify-between items-center pb-3 border-b border-zinc-100 dark:border-zinc-800">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-2xl bg-indigo-50 dark:bg-indigo-950 text-indigo-600 dark:text-indigo-400 flex items-center justify-center font-extrabold text-sm border border-indigo-200 dark:border-indigo-800">
                2
              </div>
              <div>
                <h2 className="text-base font-extrabold text-zinc-900 dark:text-zinc-100 flex items-center gap-2">
        ...
        {/* PASO 3: STACK TÉCNICO & HABILIDADES WBS (SOPORTE DE PERFIL DUAL PARA LÍDER) */}
        <div ref={step3Ref} className={`p-6 rounded-3xl bg-white dark:bg-zinc-900 border transition-all shadow-md space-y-6 ${activeStep === 3 ? 'border-blue-500 ring-2 ring-blue-500/20' : 'border-zinc-200 dark:border-zinc-800'}`}>
          <div className="flex justify-between items-center pb-3 border-b border-zinc-100 dark:border-zinc-800">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-2xl bg-emerald-50 dark:bg-emerald-950 text-emerald-600 dark:text-emerald-400 flex items-center justify-center font-extrabold text-sm border border-emerald-200 dark:border-emerald-800">
                3
              </div>
              <div>
                <h2 className="text-base font-extrabold text-zinc-900 dark:text-zinc-100 flex items-center gap-2">
                ...
                <p className="text-xs text-zinc-500 font-medium">
                  {formData.rol === 'LIDER' 
                    ? 'Perfil Híbrido Activo: Asigne competencias directivas de gestión y competencias técnicas de desarrollo'
                    : 'Asignación rápida por categorías técnicas para el algoritmo de match WBS'}
                </p>
              </div>
            </div>
            {formData.rol === 'LIDER' && (
              <span className="text-[0.68rem] font-extrabold px-3 py-1 rounded-full bg-amber-100 text-amber-900 dark:bg-amber-950 dark:text-amber-200 border border-amber-300 dark:border-amber-800 flex items-center gap-1.5">
                <Briefcase size={14} /> Doble Habilidad Habilitada (Líder + Dev)
              </span>
            )}
          </div>

          {/* SI EL ROL ES LÍDER -> MOSTRAR DOS CAMPOS SEPARADOS (DIRECTIVAS Y TÉCNICAS) */}
          {formData.rol === 'LIDER' ? (
            <div className="space-y-6">
              ...
              <div className="p-4 rounded-2xl bg-amber-50/40 dark:bg-amber-950/20 border border-amber-200 dark:border-amber-900/50 space-y-3">
                ...
                <div className="flex gap-2">
                  <input
                    ...
                    placeholder="Agregar habilidad directiva (ej. Gestión de Riesgos) y Enter..."
                ...
                <div className="flex flex-wrap gap-1.5 p-2 bg-white dark:bg-zinc-900 rounded-xl border border-amber-200/60 dark:border-amber-800/60 min-h-[36px]">
                  {habilidadesDirectivas.map(skill => (
                    <span key={skill} className="px-2.5 py-1 rounded-lg bg-amber-100 text-amber-900 dark:bg-amber-950 dark:text-amber-200 border border-amber-300 dark:border-amber-800 text-xs font-bold flex items-center gap-1.5">
                      <span>{skill}</span>
                      <button type="button" onClick={() => handleToggleDirectiva(skill)} className="hover:text-red-600 cursor-pointer">×</button>
                    </span>
                  ))}
                </div>
              ...
              <div className="p-4 rounded-2xl bg-blue-50/40 dark:bg-blue-950/20 border border-blue-200 dark:border-blue-900/50 space-y-3">
                ...
                {/* Badges seleccionados */}
                <div className="flex flex-wrap gap-1.5 p-2 bg-white dark:bg-zinc-900 rounded-xl border border-blue-200/60 dark:border-blue-800/60 min-h-[36px]">
                  {habilidadesTecnicas.map(skill => (
                    <span key={skill} className="px-2.5 py-1 rounded-lg bg-blue-100 text-blue-900 dark:bg-blue-950 dark:text-blue-200 border border-blue-300 dark:border-blue-800 text-xs font-bold flex items-center gap-1.5">
                      <span>{skill}</span>
                      <button type="button" onClick={() => handleToggleTecnica(skill)} className="hover:text-red-600 cursor-pointer">×</button>
                    </span>
                  ))}
                </div>
            ...
            /* SI EL ROL ES DESARROLLADOR O COORDINADOR -> MOSTRAR MÓDULO ÚNICO CATEGORIZADO */
            <div className="p-4 rounded-2xl bg-blue-50/40 dark:bg-blue-950/20 border border-blue-200 dark:border-blue-900/50 space-y-4">
              ...
              <div className="p-3 bg-white dark:bg-zinc-900 rounded-2xl border border-blue-200/60 dark:border-blue-800/60 min-h-[48px] flex flex-wrap gap-1.5 items-center">
                {habilidadesTecnicas.length === 0 ? (
                  <span className="text-xs text-zinc-400 italic">Seleccione sugerencias del catálogo o agregue personalizadas...</span>
                ) : (
                  habilidadesTecnicas.map(skill => (
