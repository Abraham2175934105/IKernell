...
CREATE INDEX IF NOT EXISTS idx_trabajador_email_personal ON trabajador(email_personal);
...
CREATE TABLE IF NOT EXISTS proyecto (
    ...
    estado VARCHAR(20) NOT NULL DEFAULT 'ACTIVO', -- 'ACTIVO', 'EN_PAUSA', 'COMPLETADO', 'FINALIZADO', 'INHABILITADO'
...
-- 2.8 TABLA HISTORIAL_CAMBIOS_COORDINADOR (Auditoría de reasignación y cambios por Coordinador)
CREATE TABLE IF NOT EXISTS historial_cambios_coordinador (
    id_historial BIGSERIAL PRIMARY KEY,
    proyecto_id BIGINT NOT NULL REFERENCES proyecto(id_proyecto) ON DELETE CASCADE,
    id_coordinador BIGINT,
    nombre_coordinador VARCHAR(150),
    email_coordinador VARCHAR(150),
...
CREATE INDEX IF NOT EXISTS idx_historial_coord_proyecto ON historial_cambios_coordinador(proyecto_id);
CREATE INDEX IF NOT EXISTS idx_historial_coord_fecha ON historial_cambios_coordinador(fecha_cambio DESC);

-- 2.9 TABLA HISTORIAL_REASIGNACION (Auditoría inmutable de reasignación WBS)
...
-- 2.10 TABLA SOLICITUD_CONTACTO (Leads comerciales del portal institucional)
CREATE TABLE IF NOT EXISTS solicitud_contacto (
    ...
    contador_reaperturas INT NOT NULL DEFAULT 0,
...
CREATE INDEX IF NOT EXISTS idx_solicitud_estado ON solicitud_contacto(estado);
CREATE INDEX IF NOT EXISTS idx_solicitud_fecha ON solicitud_contacto(fecha_envio DESC);

-- 2.11 TABLA MENSAJE_CHAT (Mensajería corporativa en tiempo real)
...
CREATE INDEX IF NOT EXISTS idx_chat_canal_fecha ON mensaje_chat(canal, fecha_envio ASC);
...
-- 2.12 TABLA DOCUMENTO_BIBLIOTECA (Gestor documental con visor A4 y soporte PDF/TXT/SQL/DOCX)
...
-- 2.13 TABLA TUTORIAL (Módulos de inducción y capacitación técnica)
...
-- 2.14 TABLA MICRO_SNIPPET (Repositorio de inyección de código con búsqueda difusa)
CREATE TABLE IF NOT EXISTS micro_snippet (
    ...
    comando_consola BOOLEAN NOT NULL DEFAULT FALSE,
    fecha_creacion TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP
);
...
-- 2.15 TABLA NOTICIA (Boletines informativos corporativos)
...
-- ==============================================================================
-- 3. DML - SEEDING INICIAL DE DATOS EMPRESARIALES (COMPLETO Y ACTUALIZADO)
-- ==============================================================================
...
-- 17. HISTORIAL DE AUDITORÍA DIRECTIVA Y REASIGNACIONES
DELETE FROM historial_cambios_coordinador;
INSERT INTO historial_cambios_coordinador (id_historial, proyecto_id, id_coordinador, nombre_coordinador, email_coordinador, accion, detalles, fecha_cambio, batch_id)
VALUES 
  (1, 1, 1, 'Carlos Gómez', 'carlos.coordinador@ikernell.com', 'APROBACION_PRESUPUESTO', 'Aprobado incremento presupuestal del 15% para el módulo de facturación y certificación DIAN.', NOW() - INTERVAL '15 days', 'BATCH-INIT-001'),
  (2, 2, 7, 'Roberto Silva', 'roberto.coord@ikernell.org', 'AUDITORIA_CMMI', 'Revisión y validación de estándares CMMI Nivel 3 en arquitectura de microservicios bancarios.', NOW() - INTERVAL '10 days', 'BATCH-AUDIT-002'),
  (3, 4, 14, 'Ana Ríos', 'ana.coordinador@ikernell.org', 'ASIGNACION_TALENTO', 'Incorporación de 2 desarrolladores frontend senior para acelerar el desarrollo del portal de pacientes.', NOW() - INTERVAL '5 days', 'BATCH-RES-003');

SELECT setval('historial_cambios_coordinador_id_historial_seq', (SELECT MAX(id_historial) FROM historial_cambios_coordinador));

DELETE FROM historial_reasignacion;
INSERT INTO historial_reasignacion (id_historial, actividad_id, desarrollador_anterior_id, nuevo_desarrollador_id, motivo, fecha_reasignacion)
VALUES 
  (1, 1004, 3, 6, 'Balance de carga de trabajo para optimización de transacciones concurrentes en microservicio.', NOW() - INTERVAL '12 days'),
  (2, 2002, 4, 9, 'Reasignación técnica por especialización en Circuit Breaker y patrones de resiliencia con Resilience4j.', NOW() - INTERVAL '8 days'),
  (3, 3002, 15, 10, 'Alineación de componentes UI táctiles según el nuevo sistema de diseño institucional.', NOW() - INTERVAL '4 days');

SELECT setval('historial_reasignacion_id_historial_seq', (SELECT MAX(id_historial) FROM historial_reasignacion));
